...
init_var() {
  ...
  rustdesk_server_ip="jy-rustdesk-server"
}
...
install_rustdesk_server() {
  if [[ -z $(docker ps -q -f "name=^${rustdesk_server_ip}$") ]]; then
    echo_content green "---> 安装 RustDesk Server"
  else
    echo_content skyBlue "---> 你已经安装了 RustDesk Server"
  fi
}
